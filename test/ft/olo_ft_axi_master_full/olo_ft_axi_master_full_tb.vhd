...
entity olo_ft_axi_master_full_tb is
    generic (
        runner_cfg     : string;
...
architecture sim of olo_ft_axi_master_full_tb is

    -----------------------------------------------------------------------------------------------
    -- Fixed Generics
    -----------------------------------------------------------------------------------------------
    -- The wrapper is width-agnostic wiring; width conversion is inherited from (and tested with)
    -- the base entity, so the user width equals the AXI width here.
    constant UserDataWidth_c           : natural := AxiDataWidth_g;
    ...
    -- Constants
    -----------------------------------------------------------------------------------------------
    constant WrCodewordWid_c : natural := eccCodewordWidth(UserDataWidth_c);
    constant RdCodewordWid_c : natural := eccCodewordWidth(UserDataWidth_c + 1);
    ...
    signal Wr_Data           : std_logic_vector(UserDataWidth_c - 1 downto 0)           := (others => '0');
    signal Wr_Valid          : std_logic                                                := '0';
    signal Wr_Ready          : std_logic;
    signal Wr_EccSec         : std_logic;
    signal Wr_EccDed         : std_logic;
    signal Rd_Data           : std_logic_vector(UserDataWidth_c - 1 downto 0);
    ...
    procedure pushWrData (
        ...
        increment  : natural := 1;
        beats      : natural := 1) is
        variable Data_v : unsigned(UserDataWidth_c - 1 downto 0);
    begin
        Data_v := resize(startValue, UserDataWidth_c);
    ...
    procedure expectRdBeatFlipped (
        ...
        msg        : string) is
        variable Bundle_v : std_logic_vector(UserDataWidth_c downto 0);
        variable Dec_v    : std_logic_vector(UserDataWidth_c downto 0);
...
begin
    ...
    p_control : process is
        ...
        variable Dec_v    : std_logic_vector(UserDataWidth_c - 1 downto 0);
    ...
    begin
        ...
        while test_suite loop
            ...
            ---------------------------------------------------------------------------------------
                ...
                if ImplWrite_g then
                    pushCommand(net, WrCmdMaster_c, x"00001080", ByteWidth_c);
            ...
            ---------------------------------------------------------------------------------------
                ...
                if ImplRead_g then
                    pushCommand(net, RdCmdMaster_c, x"00000200", ByteWidth_c);
            ...
            ---------------------------------------------------------------------------------------
                ...
                if ImplWrite_g then
                    pushCommand(net, WrCmdMaster_c, x"00004000", 40 * ByteWidth_c);
                ...
                if ImplRead_g then
                    pushCommand(net, RdCmdMaster_c, x"00005000", 40 * ByteWidth_c);
            ...
            ---------------------------------------------------------------------------------------
                ...
                -- A single-bit flip on the stored data codeword must be corrected before the
                -- beat reaches the AXI bus, and reported with exactly one Wr_EccSec pulse.
                ...
                if ImplWrite_g then
                    ...
                    pushCommand(net, WrCmdMaster_c, x"00002000", ByteWidth_c);
            ...
            ---------------------------------------------------------------------------------------
                ...
                if ImplWrite_g then
                    ...
                    pushCommand(net, WrCmdMaster_c, x"00002100", ByteWidth_c);
                    pushWrData(net, x"CAFE");
                    expect_single_write(net, AxiSlave_c, x"2100", unsigned(Dec_v));
                    expectWrResponse(RespSuccess);
                    check_equal(WrSecCnt, 0, "WrSecCnt");
            ...
            ---------------------------------------------------------------------------------------
                ...
                if ImplRead_g then
                    ...
                    pushCommand(net, RdCmdMaster_c, x"00003000", ByteWidth_c);
                    push_single_read(net, AxiSlave_c, x"3000", x"1A2B");
                    expectRdBeatFlipped(net, std_logic_vector(resize(unsigned'(x"1A2B"), UserDataWidth_c)), '1',
            ...
            ---------------------------------------------------------------------------------------
                ...
                if ImplRead_g then
                    ...
                    pushCommand(net, RdCmdMaster_c, x"00003100", ByteWidth_c);
                    push_single_read(net, AxiSlave_c, x"3100", x"3C4D");
                    expectRdBeatFlipped(net, std_logic_vector(resize(unsigned'(x"3C4D"), UserDataWidth_c)), '1',
    ...
    -- DUT
    -----------------------------------------------------------------------------------------------
    i_dut : entity olo.olo_ft_axi_master_full
        generic map (
            ...
            UserDataWidth_g           => UserDataWidth_c,
